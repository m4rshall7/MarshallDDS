...
const APPS_SCRIPT_URL = "https://script.google.com/macros/s/AKfycbwAujbAaMqaXCstRHEsVrDPK3Yv0nlaw5U2_eOBYgiTunb-T1z3GVVsu0TNZ6IvtrCflQ/exec";
        ...
        {step===2 && (
          <div>
            ...
            <div style={{background:accentLight,borderRadius:16,padding:'20px 16px',textAlign:'center',marginBottom:16,minHeight:80,display:'flex',alignItems:'center',justifyContent:'center'}}>
              <span style={{fontSize:40,fontWeight:700,color:accentColor,letterSpacing:2}}>{amount || '0'} ₽</span>
            </div>
            ...
            <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:10,marginBottom:16}}>
              {['1','2','3','4','5','6','7','8','9','.','0','del'].map(k=>(
                <button key={k} onClick={()=>handleAmount(k)}
                  style={{padding:'18px 0',fontSize:22,fontWeight:k==='del'?400:600,background:k==='del'?'#f1f5f9':'#fff',border:'1.5px solid #e2e8f0',borderRadius:12,cursor:'pointer',color:'#1A1A2E'}}>
                  {k==='del'?'⌫':k}
                </button>
              ))}
            </div>
          ...
          <div>
            ...
            <div style={{background:accentLight,borderRadius:20,padding:24,marginBottom:20,border:`2px solid ${accentColor}`}}>
              ...
              <div style={{display:'flex',flexDirection:'column',gap:10}}>
                {[['📅 Дата',date],['🏷 Статья',`${cat.emoji} ${cat.name}`],['📊 Тип',txType],['🏗 Деятельность',cat.activity]].map(([k,v])=>(
            ...
            <button onClick={handleSubmit} disabled={loading}
              style={{width:'100%',padding:18,borderRadius:16,border:'none',background:accentColor,color:'#fff',fontSize:17,fontWeight:700,cursor:'pointer',boxShadow:`0 4px 16px ${accentColor}44`}}>
              {loading?'⏳ Сохраняем...':'✅ Сохранить операцию'}
            </button>
            <button onClick={()=>setStep(3)}
              style={{width:'100%',padding:14,marginTop:10,borderRadius:12,border:'1.5px solid #e2e8f0',background:'#fff',fontSize:15,color:'#64748b',cursor:'pointer'}}>
              ← Изменить статью
            </button>
          </div>
        )}
...
function DashboardScreen({ transactions }) {
  ...
  const monthTx = transactions.filter(tx => {
    try { const d = parseISO(tx.date); return isWithinInterval(d,{start:monthStart,end:monthEnd}); }
    catch { return false; }
  });
  ...
  const catTotals = {};
  monthTx.filter(t=>t.type==='Выплата').forEach(t=>{ catTotals[t.category]=(catTotals[t.category]||0)+t.amount; });
  ...
  return (
    <div style={{padding:'0 16px 16px',fontFamily:'Arial,sans-serif'}}>
      ...
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10,marginBottom:10}}>
        {[{label:'Поступления',val:income,color:'#1B6B40',bg:'#D5F5E3',icon:'📈'},{label:'Расходы',val:expense,color:'#922B21',bg:'#FADBD8',icon:'📉'}].map(k=>(
          <div key={k.label} style={{background:k.bg,borderRadius:16,padding:'16px 14px',border:`1.5px solid ${k.color}22`}}>
      ...
      <div style={{background:balance>=0?'#0E6655':'#922B21',borderRadius:16,padding:'18px 16px',marginBottom:16,display:'flex',justifyContent:'space-between',alignItems:'center'}}>
        <div>
      ...
      {topCats.length>0 && (
        <div style={{background:'#f8fafc',borderRadius:16,padding:'16px',marginBottom:16}}>
          <div style={{fontSize:13,fontWeight:700,color:'#1A1A2E',marginBottom:12}}>📊 Топ расходов за месяц</div>
...
function HistoryScreen({ transactions, onDelete }) {
  ...
  const grouped = {};
  filtered.forEach(t=>{ const key=t.date||'unknown'; if(!grouped[key])grouped[key]=[]; grouped[key].push(t); });
      ...
      <div style={{display:'flex',gap:8,marginBottom:16}}>
        {[['all','Все'],['income','Приход'],['expense','Расход']].map(([v,l])=>(
          <button key={v} onClick={()=>setFilter(v)}
            style={{padding:'8px 16px',borderRadius:20,border:'none',fontSize:13,fontWeight:600,background:filter===v?'#1A1A2E':'#f1f5f9',color:filter===v?'#fff':'#64748b',cursor:'pointer'}}>
            {l}
          </button>
        ))}
      </div>
      ...
      {sortedDates.map(date=>(
        <div key={date} style={{marginBottom:16}}>
          ...
          {grouped[date].map(tx=>{
            ...
            return (
              <div key={tx.id} style={{background:'#fff',borderRadius:14,padding:'14px 16px',marginBottom:8,boxShadow:'0 1px 4px rgba(0,0,0,0.08)',display:'flex',alignItems:'center',gap:12,border:`1px solid ${isIn?'#D5F5E3':'#FADBD8'}`}}>
                <div style={{width:42,height:42,borderRadius:12,background:isIn?'#D5F5E3':'#FADBD8',display:'flex',alignItems:'center',justifyContent:'center',fontSize:20,flexShrink:0}}>{cat?.emoji||'💰'}</div>
                <div style={{flex:1,minWidth:0}}>
                  ...
                  <div style={{fontSize:11,color:tx.synced?'#1B6B40':'#F39C12',marginTop:2}}>{tx.synced?'✓ синхр.':'● не синхр.'}</div>
                </div>
                <div style={{textAlign:'right',flexShrink:0}}>
                  <div style={{fontSize:16,fontWeight:700,color:isIn?'#1B6B40':'#922B21'}}>{isIn?'+':'-'}{fmt(tx.amount)} ₽</div>
                  <button onClick={()=>onDelete(tx.id)} style={{background:'none',border:'none',color:'#e2e8f0',fontSize:16,cursor:'pointer',padding:'4px 0',marginTop:2}}>✕</button>
                </div>
              </div>
            );
          })}
  ...
  const handleSync = async () => {
    ...
    try {
      const rows = unsynced.map(t=>({
        ...
        activity: t.activity, amount: t.amount, comment: t.comment||'', id: t.id,
      }));
      // Прямой запрос к Apps Script (no-cors для Google)
      const resp = await fetch(APPS_SCRIPT_URL, {
        method: 'POST',
        headers: { 'Content-Type': 'text/plain' },
        body: JSON.stringify({ transactions: rows }),
      });
      // Apps Script возвращает JSON
      ...
      // CORS ошибка от Apps Script — но данные всё равно записались
      await onSynced(unsynced.map(t=>t.id));
      setResult({ok:true, msg:`✅ ${unsynced.length} операций отправлено в Google Sheets!`});
  ...
  return (
    <div style={{padding:'0 16px 16px',fontFamily:'Arial,sans-serif'}}>
      <div style={{background:unsynced.length===0?'#D5F5E3':'#FEF9E7',borderRadius:16,padding:'20px 16px',marginBottom:20,border:`1.5px solid ${unsynced.length===0?'#1B6B40':'#F39C12'}`,textAlign:'center'}}>
        ...
        <div style={{fontSize:13,color:'#64748b'}}>{unsynced.length===0?'Все операции отправлены в Google Sheets':'Нажми кнопку чтобы отправить в Google Sheets'}</div>
      </div>
      ...
      {result&&(
        <div style={{background:result.ok?'#D5F5E3':'#FADBD8',borderRadius:12,padding:'14px 16px',marginBottom:16,fontSize:14,color:result.ok?'#1B6B40':'#922B21',fontWeight:600}}>{result.msg}</div>
      )}
      <div style={{background:'#f8fafc',borderRadius:16,padding:16}}>
        ...
        {[['Всего операций',transactions.length],['Синхронизировано',transactions.filter(t=>t.synced).length],['Ожидают',unsynced.length]].map(([k,v])=>(
...
export default function App() {
  ...
  const handleAdd = useCallback((tx) => { setTxs(prev=>[tx,...prev]); showToast('Операция сохранена! 💾'); }, []);
  ...
  return (
    <div style={{maxWidth:430,margin:'0 auto',minHeight:'100vh',background:'#f8fafc',fontFamily:'Arial,sans-serif',position:'relative'}}>
      ...
      <div style={{background:'#1A1A2E',color:'#fff',padding:'16px 20px 12px',display:'flex',justifyContent:'space-between',alignItems:'center',position:'sticky',top:0,zIndex:50}}>
        ...
        <button onClick={()=>setShowAdd(true)} style={{background:'#0F3460',border:'none',color:'#fff',width:48,height:48,borderRadius:14,fontSize:26,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',boxShadow:'0 2px 8px rgba(15,52,96,0.5)'}}>+</button>
      </div>
        ...
        {tabs.map(t=>(
          <button key={t.id} onClick={()=>setTab(t.id)} style={{flex:1,padding:'10px 0',border:'none',background:'none',cursor:'pointer',display:'flex',flexDirection:'column',alignItems:'center',gap:3,position:'relative'}}>
            ...
            {t.id==='sync'&&unsyncedCount>0&&(
              <div style={{position:'absolute',top:6,right:'calc(50% - 18px)',background:'#F39C12',color:'#fff',fontSize:10,fontWeight:700,width:16,height:16,borderRadius:'50%',display:'flex',alignItems:'center',justifyContent:'center'}}>{unsyncedCount}</div>
            )}
            {tab===t.id&&<div style={{position:'absolute',top:0,left:'20%',right:'20%',height:3,background:'#0F3460',borderRadius:2}}/>}
          </button>
        ))}
      </div>
      {showAdd&&<AddScreen onAdd={handleAdd} onClose={()=>setShowAdd(false)}/>}
